...
// SetFollowUpModule stores a reference to the FollowUpModule for voice delegation
...
func (m *VoiceModule) Register() {
        ...
        // ✨ Improve draft with AI (как в текстовом флоу /new)
        voiceAIBtn := tb.InlineButton{Unique: "vc_ai"}
        m.bot.Handle(&voiceAIBtn, safeHandler("vc_ai", m.handleImprove))
...
func (m *VoiceModule) showDraft(c tb.Context, sess *session.SessionData) error {
        ...
        kb.InlineKeyboard = [][]tb.InlineButton{
                ...
                {{Unique: "vc_ai", Text: "✨ Покращити з AI"}},
...
// handleImprove — «✨ Покращити з AI»: улучшает тему и текст черновика
// через Gemini (тот же ImproveRequest, что и в текстовом флоу /new).
func (m *VoiceModule) handleImprove(c tb.Context) error {
        _ = c.Respond()
        sess := c.Get("session").(*session.SessionData)
        if m.deps.Gemini == nil || !m.deps.Gemini.Available() {
        ...
        saveSession(m.deps, c)
        _ = c.Send("✨ Чернетку покращено AI!")
        return m.showDraft(c, sess)
}
