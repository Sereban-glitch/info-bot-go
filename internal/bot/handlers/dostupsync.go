...
func (w *DostupSync) notifyResponse(e sentlog.SentEntry, st *dostup.RequestStatus) {
        ...
        // Вложения ответа: органы часто присылают PDF — пользователь сразу
        // видит, что суть в файле, а кнопка розбора этот PDF прочитает.
        if len(st.Attachments) > 0 {
                names := strings.Join(st.Attachments, ", ")
                if len(names) > 200 {
                        names = names[:200] + "…"
                }
                text += fmt.Sprintf("📎 Вкладлення: %s\n\n", htmlEscape(names))
        }
