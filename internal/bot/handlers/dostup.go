...
func (m *DostupModule) handleSubmit(c tb.Context) error {
	...
	// ВАЖНО: editMessageText (c.Edit) принимает ТОЛЬКО inline-клавиатуры.
	// Раньше сюда передавалась MainMenuKeyboard — reply-клавиатура — и
	// Telegram отвечал «inline keyboard expected (400)»: запрос публико-
	// вался на портале, но пользователь оставался у зависшего «⏳ Подаю
	// запит…» без ссылки и дедлайна (реальный случай 29.08.2026, МОЗ).
	// Главное reply-меню и так постоянно открыто в чате, поэтому здесь
	// клавиатуру не передаём вовсе: Telegram снимет кнопку «Надіслати»
	// с отредактированного сообщения. Даже если кнопка где-то останется —
	// черновик уже сброшен, повторное нажатие безопасно («почніть заново»).
