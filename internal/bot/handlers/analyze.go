...
func (m *AnalyzeModule) handleStart(c tb.Context) error {
        ...
        text := "⚖️ <b>Розбір відповіді органу</b>\n\n" +
                ...
                "• 📄 <b>PDF-файл</b> — якщо орган надіслав відповідь вкладенням;\n" +
...
func (m *AnalyzeModule) HandleMedia(c tb.Context) error {
        ...
        msg := c.Message()
        if msg == nil || (msg.Photo == nil && msg.Document == nil) {
                return c.Send("📷 Надішліть фото листа, PDF-файл або текст відповіді. Скасувати: /cancel")
        }
        if m.deps.Gemini == nil || !m.deps.Gemini.Available() {
                return c.Send("⚠️ AI-розбір зараз недоступний. Спробуйте пізніше.")
        }

        // PDF-файл от пользователя (орган прислал PDF, пользователь
        // перетащил его в бот): скачиваем и извлекаем текст.
        if msg.Document != nil {
                return m.handleDocumentPDF(c, sess)
        }
...
// handleDocumentPDF — пользователь прислал файл в шаге розбора.
// PDF: скачиваем через file API бота, извлекаем текст, анализируем.
// Прочие форматы: подсказка (фото читает AI, текст — просто текстом).
func (m *AnalyzeModule) handleDocumentPDF(c tb.Context, sess *session.SessionData) error {
        msg := c.Message()
        doc := msg.Document
        name := doc.FileName
        lower := strings.ToLower(name)
        isPDF := doc.MIME == "application/pdf" || strings.HasSuffix(lower, ".pdf")
        if !isPDF {
                return c.Send("📄 Підтримую PDF-документи. Надішліть відповідь органу як PDF, фото листа або просто текстом.")
        }

        _ = c.Send("📎 Читаю PDF-файл…")
        data, err := m.downloadDocument(c, doc)
        if err != nil {
                log.Printf("[ANALYZE] document download: %v", err)
                return c.Send("❌ Не вдалося завантажити файл. Спробуйте ще раз або надішліть текстом.")
        }
        if int64(len(data)) > pdftext.MaxBytes {
                return c.Send("❌ Файл завеликий. Надішліть PDF менше 15 МБ або фото сторінок листа.")
        }
        text, err := pdftext.Extract(data, 8000)
        if err != nil {
                if errors.Is(err, pdftext.ErrNoText) {
                        return c.Send("📄 Цей PDF — скан без текстового шару. Надішліть, будь ласка, фото сторінок листа — розберу його з зображення.")
                }
                log.Printf("[ANALYZE] pdftext %s: %v", name, err)
                return c.Send("❌ Не вдалося прочитати PDF (файл пошкоджений або захищений). Спробуйте фото або текстом.")
        }
        ...
        saveSession(m.deps, c)
        log.Printf("[ANALYZE] user PDF %s: %d знаков текста", name, utf8RuneCount(text))
        return m.runAnalysis(c, sess, nil)
}

// downloadDocument скачивает документ (file API бота) — как downloadPhoto,
// но для присланных файлов; лимит 15 МБ с запасом под лимит Telegram.
func (m *AnalyzeModule) downloadDocument(c tb.Context, doc *tb.Document) ([]byte, error) {
        if doc == nil {
        ...
        if resp.StatusCode != http.StatusOK {
                return nil, fmt.Errorf("document download: HTTP %d", resp.StatusCode)
        }
        const maxDocBytes = 15 << 20 // 15 МБ
        data, err := io.ReadAll(io.LimitReader(resp.Body, maxDocBytes+1))
        if err != nil {
                return nil, err
        }
        if int64(len(data)) > maxDocBytes {
                return nil, fmt.Errorf("документ більше 15 МБ")
        }
        return data, nil
}
...
// из гилки портала. Ответ органа часто приходит PDF-вложением (текст
// письма — только подпись): скачиваем такие вложения и добавляем их
// текст к разбору, чтобы AI видел суть ответа, а не «З повагою…».
func (m *AnalyzeModule) handleFromNotification(c tb.Context) error {
        ...
        replyText, err := m.deps.Dostup.GetRequestResponseText(slug, 12000)
        if err != nil {
...
func (m *AnalyzeModule) appendPDFAttachments(c tb.Context, slug, replyText string) (string, int, []string) {
        ...
        _ = c.Send(fmt.Sprintf("📎 Відповідь містить PDF-вкладення (%s) — завантажую та читаю текст…",
                strings.Join(names, ", ")))
        ...
        for _, a := range pdfs {
                ...
                if derr != nil {
                        log.Printf("[ANALYZE] download %s: %v", a.Name, derr)
...
// нужен, чтобы оценить, есть ли в тексте что-то кроме него и подписи.
func stripAttachmentMarker(s string) string {
        if i := strings.Index(s, "[Вкладення:"); i >= 0 {
