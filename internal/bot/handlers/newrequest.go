...
// newFOITempPath создаёт уникальный временный файл для PDF и возвращает
// путь + функцию очистки. Уникальность гарантирована os.CreateTemp —
// два одновременных запроса никогда не пересекаются (фикс гонки A2).
func newFOITempPath() (string, func(), error) {
	tmp, err := os.CreateTemp("", "foi_*.pdf")
	if err != nil {
...
func generateFOIRequestPDF(data email.RequestData) ([]byte, error) {
	// Security fix (audit #22, A2): unique temp file per request.
	// The old fixed path /tmp/foi_request.pdf caused a cross-user race —
	// two simultaneous requests could attach each other's PDF.
	tmpPath, cleanup, err := newFOITempPath()
	if err != nil {
		return nil, err
	}
	defer cleanup()
