...
// analysisMaxReplyLen — максимум РУН текста ответа органа в запросе
// (кириллица занимает 2 байта; байтовая нарезка ломала бы UTF-8).
const analysisMaxReplyLen = 8000

// TruncateReplyText — рун-безопасное усечение текста ответа для AI.
// Если текст длиннее analysisMaxReplyLen рун: сохраняем начало (70%)
// и конец (30%) — в конце письма обычно даты, строки и подпись, нужные
// для оценки сроков. Начало и конец разделяет маркер «[…]».
// Экспортировано для веб-API (/api/analyze) — там та же защита.
func TruncateReplyText(s string) string {
        rn := []rune(s)
        if len(rn) <= analysisMaxReplyLen {
                return s
        }
        head := analysisMaxReplyLen * 7 / 10
        tail := analysisMaxReplyLen - head
