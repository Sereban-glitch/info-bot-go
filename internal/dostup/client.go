...
type RequestStatus struct {
        ...
        ResponseExcerpt string `json:"responseExcerpt,omitempty"` // первые ~400 символов ответа (без мусора вложений)
        ...
        Attachments     []string `json:"attachments,omitempty"`    // имена файлов последнего ответа («RS.pdf»…)
}
...
func (c *Client) GetRequestStatus(slug string) (*RequestStatus, error) {
        ...
        if len(incoming) > 0 {
                ...
                // вложения последнего ответа (имена — для уведомлений и розбора)
                if atts := parseAttachments(tail); len(atts) > 0 {
                        for _, a := range atts {
                                st.Attachments = append(st.Attachments, a.Name)
                        }
                }
        }
        return st, nil
...
// Блок вложений вырезается, вместо него — маркер «[Вкладення: …]»:
// мусор «3 Attachments RS.pdf View Download» больше не попадает в AI-промпт.
...
// cleanCorrespondenceHTML — HTML-фрагмент письма органа → чистый текст:
// вырезаем блок вложений, добавляем маркер «[Вкладення: …]», снимаем теги,
// разжимаем сущности, схлопываем пробелы.
func cleanCorrespondenceHTML(html string) string {
        atts := parseAttachments(html)
        html = stripAttachmentsHTML(html)
        text := htmlUnescape(reTag.ReplaceAllString(html, " "))
        text = strings.Join(strings.Fields(text), " ")
        if marker := attachmentMarker(atts); marker != "" {
                if text == "" {
                        text = marker
                } else {
                        text = text + " " + marker
